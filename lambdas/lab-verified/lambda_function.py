...
ALLOWED_GROUPS = {'receiving-lab'}
REQUIRED_PRIOR_STATUS = 'DELIVERED'
NEW_STATUS = 'VERIFIED'


def log(level, message, **fields):
    print(json.dumps({'level': level, 'message': message, 'endpoint': 'lab-verified', **fields}))
...
def lambda_handler(event, context):
    ...
    if not any(group in ALLOWED_GROUPS for group in caller_groups):
        log('WARN', 'Rejected unauthorized verification attempt',
            lambda_request_id=lambda_request_id, caller_sub=caller_sub)
        return {'statusCode': 403, 'body': json.dumps({'error': 'Caller is not authorized to verify specimen receipt'})}
    ...
    if current_item.get('current_status') != REQUIRED_PRIOR_STATUS:
        ...
        return {'statusCode': 409, 'body': json.dumps({
            'error': f"Cannot verify. Current status: {current_item.get('current_status')}, expected: {REQUIRED_PRIOR_STATUS}"
        })}

    event_type = "LabVerified"
    ...
    try:
        ...
        log('ERROR', 'Failed to write LabVerified event', lambda_request_id=lambda_request_id, error=str(e))
        return {'statusCode': 500, 'body': json.dumps({'error': 'Failed to record verification, please try again'})}
    ...
    try:
        ...
        eventbridge.put_events(Entries=[{
            ...
            'Detail': json.dumps({'specimen_id': specimen_id, 'event_type': event_type, 'verified_by': caller_sub, 'timestamp': timestamp}),
    ...
    log('INFO', 'Lab verification recorded successfully', lambda_request_id=lambda_request_id, specimen_id=specimen_id,
        duration_ms=round((time.time() - start_time) * 1000, 2))
    return {'statusCode': 200, 'body': json.dumps({'message': 'Lab verification recorded', 'specimen_id': specimen_id})}